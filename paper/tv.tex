...
Intuitively, the Nullable function \code{\delta(\effect)} returns a boolean value indicating whether \code{\effect} contains the empty trace; the First function \code{fst_{{\pi}}( \effect)} computes a set of possible initial time instances of \code{{\pi} : \effect}; and the Derivative function \code{D_{I\mysharp  t}^{{\pi}}(\effect)} computes a next-state effects after eliminating one event \code{I} w.r.t. the execution time \code{t} from the current effects \code{{\pi} : \effect}. 
...
possible initial events of \code{{\pi} \wedge \es}; and the Derivative 
function \code{D_{\anyevent{a}}^{{\pi}}(\es)} computes a next-state 
effects after eliminating one event \code{\anyevent{a}} from the current 
...
Given any timed sequence \code{\es} under condition \code{{\pi}}, we define \code{\delta_{{\pi}}(\es)} to be:\\
{
...
\begin{definition}[First]\label{First}
  Let \code{fst_{{\pi}}(\es): \{ (\ev,  t)  | 
  \pi {\wedge} t{\geq}0 \wedge (\ev \mysharp  t) {\cdot} \es_1 \in \llbracket  
  \pi \wedge \es \rrbracket \}} be the set of initial timed events  
  derivable from effects \code{\pi \wedge \es}. (\code{\llbracket  \pi \wedge
   \es \rrbracket } represents all the timed  traces contained in 
   \code{\pi \wedge \es}) 
  { 
   \begin{gather*} 
  \code{fst_{{\pi}}( \bott) {=} \{\} } \qquad\quad
  \code{fst_{{\pi}}(\empt) {=}  \{\} } \qquad\quad
  \code{fst_{{\pi}}( \ev) {=} \{ (\ev, t) \}  \ (t\ is\ fresh)} 
  \qquad\quad
     \code{fst_{{\pi}}(  \es \mysharp  t) {=}  fst_{\pi}(\es)}
   \\
