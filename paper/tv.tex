...
\code{D^{\env}_{h}(\pi \wedge \es_1 {||}\es_2)} {=} \code{PD^{\env}_{h}(\pi \wedge \es_1)} || \code{PD^{\env}_{h}(\pi \wedge \es_2)} 
 ...
 \hline
%\\
%\code{D^{\env}_{h}(  \effect_1 \ ||\  \effect_2) } {=} \code{ D^{\env}_{h}(  \effect_1) \ ||\ D^{\env}_{h}(  \effect_2)} 
\\
\code{D^{\env}_{h}(  \es \mysharp  t) } {=}
\code{
\pi \wedge(t_1{+} t_2 {=}t) \wedge (t_1{=}t^\prime) : (D^{\env}_{h}(\effect))\mysharp  t_2} \ \ \ 
\code{(fresh \ t_1\ t_2 )}
\\
\code{D^{\env}_{h}(\pi^\prime? \es)} {=} \code{\pi: \empt\ (}\code{ I {\subseteq} \{\textbf{S}\})}
\qquad
\code{D^{\env}_{h}(\textbf{S}?)} {=} \code{\pi: \textbf{S}?\ (}\code{ I {\not\subseteq} \{\textbf{S}\})}
\\
\code{D^{\env}_{h}(J)} {=} \code{\pi: \empt\ (}\code{ I {\subseteq} J)}
\qquad\ 
    \code{D^{\env}_{h}( J)} {=} \code{False : \bott \ (}\code{I {\not\subseteq} J)} 
\end{gather*}
\end{definition}
