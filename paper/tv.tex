...
\item \lstinline|void addSugar (int n) {| \textcolor{mGray}{\footnotesize \emph{(-- initialize the current effects state --)}} \\
     \textcolor{darkpastelred}{\{\code{\effect_C {=}\effect_{pre}^{addSugar(n)} =  n{\geq}0 \wedge  \_^\star \} }} 
      \siderule{\scriptsize[FV\text{-}Meth] } \\
     \item ~\qquad \lstinline|if (n == 0) {| \\
     \textcolor{darkpastelred}{\code{\{ n{=}0 \wedge \anyevent{Ready} \cdot \_^\star \}  }} \siderule{\scriptsize [FV\text{-}If-Else]}
      \item ~\qquad~\qquad \lstinline|event ["Sugar"];} |  \\
     \textcolor{darkpastelred}{\code{\{n{=}0 \wedge \anyevent{Ready} \cdot \_^\star \cdot \anyevent{Done}\} }} \siderule{\scriptsize [FV\text{-}Event]}
     \item 
     ~\qquad \lstinline|else {| \\
     \textcolor{darkpastelred}{\code{\{ n  {\neq} 0\}  }} \siderule{\scriptsize [FV\text{-}If-Else]}
    \item 
     ~\qquad~\qquad \lstinline|timeout (oneSugar() , 1);| \\
     \textcolor{darkpastelred}{\code{\{n  {\neq} 0  \wedge \anyevent{Ready} \cdot \_^\star \cdot \anyevent{Send}  }\}} \siderule {\scriptsize [FV\text{-}Event]}
     \item 
     ~\qquad~\qquad \lstinline|addSugar (n-1);}}|  \\
      \code{rev(n  {\neq} 0  \wedge \anyevent{Ready} \cdot \_^\star \cdot \anyevent{Send})  \CONTAIN rev(\effect_{pre}^{send(n\text{-}1)}) }
      \textcolor{mGray}{  \emph{(-check precondition-) }}\\
     \textcolor{darkpastelred}{\code{\{ n  {\neq} 0  \wedge \anyevent{Ready} \cdot \_^\star \cdot \anyevent{Send}\cdot \effect_{post}^{send(n\text{-}1)} }\}} \siderule {\scriptsize [FV\text{-}Call]}
      \item \textcolor{darkpastelred}{\code{\effect_C^\prime {=} (n{=}0 \wedge \anyevent{Ready} \cdot \_^\star \cdot  \anyevent{Done})\ \vee\ }\code{ (n  {\neq} 0  \wedge \anyevent{Ready} \cdot \_^\star \cdot \anyevent{Send}\cdot \effect_{post}^{send(n\text{-}1)}) }} 
      \\
       \item \code{
       %\code{(n{=}0 \wedge \anyevent{Done})\ \vee\ }\code{ (n  \neq 0  \wedge \anyevent{Send}\cdot \effect_{post}^{send(n-1)}) } 
       \effect_C^\prime \CONTAIN \effect_{pre}^{send(n)} \cdot \effect_{post}^{send(n)} \Leftrightarrow   \textcolor{mGray}{  \emph{(-- check postcondition --) }}}
        \\ \code{\code{\code{(n{=}0 \wedge \anyevent{Done})\ \vee\ }\code{ (n  {\neq} 0  \wedge \anyevent{Send}\cdot \effect_{post}^{send(n\text{-}1)}) } \CONTAIN \effect_{post}^{send(n)}}}\\
