...
inclusions among \timedEffects\ (cf. \defref{Inclusion} and 
...
\code{\effect_1,\ \effect_2}, TRS decides if the inclusion \code{\effect_1 \CONTAIN  \effect_2 } is valid. 
...
%\code{\delta_{{\pi}}(\es)} to be \\
{
  ...
  \begin{cases}
        \code{true} & \code{if\  \epsilon \in \llbracket {\pi}  \wedge  \es \rrbracket_{\varphi}}\\
...
\begin{definition}[First]\label{First}
  ...
  derivable from effects \code{\pi \wedge \es}, where: 
    { 
   \begin{gather*} 
  \code{fst_{{\pi}}( \bot ) {=} \{\} } \qquad\ \ 
  \code{fst_{{\pi}}( \epsilon) {=}  \{\} } \qquad\ \ 
  \code{fst_{{\pi}}( \ev) {=} \{ \ev \}  }
  \qquad\ \ 
...
\begin{gather*}
\code{D^{\env}_{h}(\pi \wedge \bot ) {=}}  \code{False \wedge \bot } 
\qquad\ \ \  
\code{D^{\env}_{h}( \pi \wedge  \epsilon) {=}}  \code{False \wedge \bot } 
\qquad\ \ \ 
...
\\
\code{D^{\env}_{h}(\pi \wedge  \es_1 {\vee} \es_2) } {=} \code{ D^{\env}_{h}(\pi \wedge  \es_1) \vee D^{\env}_{h}( \pi \wedge \es_2)} 
\qquad 
\code{D^{\env}_{h}(\pi \wedge \es_1 {||}\es_2)} {=} \code{PD^{\env}_{h}(\pi \wedge \es_1)} || \code{PD^{\env}_{h}(\pi \wedge \es_2)} 
 \\~\\
 \hline
 ~\\
 \code{D^{\env}_{t}(\pi \wedge  \ev) } {=} \code{False \wedge \bot } 
 \qquad\qquad 
...
\begin{definition}[Parallel Derivative]\label{Derivative_maybe}
  ...
  the parallel derivative is defined as: {
    ...
    \begin{cases}
          \code{\pi \wedge \es} & \code{if\ D^{\env}_{h}(\pi \wedge \es) = (False \wedge \bot )}\\
...
%  &\code{\bot  \cdot \effect \rightarrow \bot  }  
%  &
% 
%  
%\\ \hline
%
%  \code{\bot  \vee \effect \rightarrow \effect } & \code{ \effect  \cdot \bot  \rightarrow \bot  }  &
%
...
%&\code{\bot  ^\star \rightarrow  \epsilon}
%& 
%
%
%
...
%
%
% \code{ \epsilon \cdot \effect  \rightarrow \effect }
...
\hline
(A15)  &
\code{ \effect\ ||\  \epsilon \rightarrow \effect}\\
\hline
(A16)  &
\code{ \effect\ ||\ \bot  \rightarrow \bot }\\
\hline
(A17)  &
...
\hline
(A19)  &
 \code{ \epsilon \mysharp  t \rightarrow  \epsilon}\\
\hline
(A20)  &
 \code{\bot \mysharp  t \rightarrow  \bot }\\
\hline
(A21)  &
 \code{{{False}} : \effect \rightarrow {{False}} : \bot }\\
\hline
(A22)  &
 \code{{\pi} : \bot  \rightarrow {{False}} : \bot }\\
\hline
 %\code{\effect \vee \effect \rightarrow \effect} 
%  &\code{\bot  \cdot \effect \rightarrow \bot  }  
%  &
