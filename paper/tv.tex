...
\figref{fig:Syntax_of_Types_and_Effects} Effects is 
 a conditioned time  event sequence \code{\pure{\pi},  \es} or a disjunction of two effects $\effect_1 \vee \effect_2$.
...
We here present the forward verification rules, s
...
triples \code{ \Pi, \env \vdash \s{ \effect_c} \ e\ \s{\effect_c^\prime}}
where 
\code{\Pi} is the current guard, 
\code{\env} is the current valuation of the variables;
\code{\effect_c} is the current effects, 
and \code{ \effect_C^\prime}
is the resulting effects by executing \code{e}. 
...
\begin{gather*}
  \code{\frac{\effect_C^\prime {=} \effect_C\cdot \anyevent{a}}{\vdash \{  \effect_C \}\  \textbf{event} [\anyevent{a}] \ \{ \effect_C^\prime \}} \   {[FV\text{-}Event]}} 
  \qquad
  \code{\frac{\vdash \{ \effect_C\}\ e_1 \ \{  \effect_C^\prime\} \qquad \vdash \{  \effect_C^\prime\}\ e_2 \ \{  \effect_C^{\prime\prime}\} }{\vdash \{  \effect_C\}\  e_1;e_2 \ \{  \effect_C^{\prime\prime}\}} \  {[FV\text{-}Seq]}}  
\end{gather*}
...
\begin{gather*}
\code{\frac{ \vdash \{  v \wedge \effect_C  \} \ e_1 \ \{  \effect_C^{\prime}\} \qquad \vdash \{ \neg v \wedge \effect_C \} \ e_2 \ \{  \effect_C^{\prime\prime}\}  }
{\vdash \{  \effect_C\}\  \textbf{if}\ v\ \textbf{then}\ e_1\ \textbf{else}\ e_2 \ \{  \effect_C^{\prime} \vee\effect_C^{\prime\prime} \} } \  {[FV\text{-}If\text{-}Else]}} 
\end{gather*}
...
\\
\text{\code{\vdash rev( \effect_C) \CONTAIN rev([y^*/x^*] \effect_{pre})   \qquad   \effect_C^\prime =  \effect_C \cdot  [y^*/x^*]\effect_{post}}}
\end{matrix}}
{\vdash \{  \effect_C \} \  mn({y^*})  \ \{  \effect_C^\prime\}} \ {[FV\text{-}Call]}}
\end{gather*}

\begin{gather*}
\code{\frac{  \vdash \{  \empt \}  \ e \  \{ \effect_C \} \qquad\qquad  \vdash\effect_C \CONTAIN   \effect_{post}}{  \vdash \tau \ mn  \ {( \tau\ x)^*}\ \{\textbf{requires}\ \effect_{pre}\ \textbf{ensures}\  \effect_{post}\} \ \{ e \}} \ {[FV\text{-}Meth]}}  
\end{gather*}
