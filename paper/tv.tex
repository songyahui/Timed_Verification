...
to be successful \cite{DBLP:journals/tse/WangSWQ17,DBLP:journals/sttt/LarsenPY97,yovine1997kronos,wang2005verifying}.
...
Works based on such a TRS \cite{song2020automated,DBLP:journals/tcs/AntimirovM95,almeida2009antimirov,keil2014symbolic,hovland2012inclusion}  show its feasibility and suggest that this method is a better average-case algorithm than those based on the comparison of automata. 
...
The preconidtion of \codem{main} requies no arithmetic constraints 
...
captured by any prior works \cite{DBLP:journals/tse/WangSWQ17,DBLP:journals/sttt/LarsenPY97,yovine1997kronos,wang2005verifying}.
...
\begin{prooftree}
\Hypo{
  \code{\textcircled{4}\siderule{[\codeme{PROVE}]}}
}
...
\Infer[dashed]1[{\textcircled{3}\siderule{[\codeme{UNFOLD}]}} \textcolor{blue}{\code{\pi_u : true}}]{ \code{ \textcolor {darkred}{n{=}0} \wedge Sugar
\CONTAIN \textcolor {darkred}{t_r{\geq}0} \wedge Sugar \mysharp t_r}}
...
In step \code{\textcircled{5}}, from the RHS, since \code{\_^\star {=} emp \vee \_ \cdot \_^\star}, eliminating one event \code{\{Ready\}} from it will lead to \code{\bot \vee \_^\star}, which can be normalized into \code{\_^\star} in step \code{\textcircled{6}}.
...
The following models Fischer's mutual exclusion algorithm. 
where d1 and d2 are two integer constants with d1 < d2; 
x and ct are global variables. The protocol is modeled as 
process Protocol, which is the parallel composition of three 
processes. Each of the three processes attempts to enter 
the critical section when x is -1, i.e. no other process is 
currently attempting. Once the process is active, it sets x 
to its identity i within d1 time units (captured by deadline[d1]). 
Then it idles for d2 time units (captured by Wait[d2]) and then 
checks whether x is still i. If so, it enters the critical 
...
%Antimirov and Mosses' algorithm \cite{DBLP:journals/tcs/AntimirovM95} was designed for deciding the inclusion of regular expressions.   
...
Yet our timed effects further capture the time bounds constraints by adding the operator \code{\mysharp }, adapting to real-time system modeling and verification, which is not covered by any existing KA variants/extensions. 
...
Trace-based analyses have been shown capable of statically enforcing flow-sensitive security properties such as safe locking behavior \cite{foster2002flow} and resource usage policies such as file usage protocols and memory management \cite{marriott2003resource}. In \cite{bartoletti2005enforcing}, a trace effect analysis is used to enforce secure service composition. 
...
Besides, prior work \cite{nielson1998behaviour} proposes an annotated type and effect system and infers behaviors from CML \cite{reppy1993concurrent} programs for channel-based communications, though it did not provide any inclusion solving process. 
