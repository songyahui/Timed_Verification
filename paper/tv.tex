...
Both preconditions of \codem{addNSugar(n)} requires the 
input  \codem{n} is non-negative, and has no temporal constrains upon 
the history traces. 
...
traditional modelling languages for real-time systems.
...
%which tightly The semantics model closely captures the temporal behavior of the program, 
...
in \figref{fig:overview_coffee}. This example simulate a 
...
function says that the input value \codem{n} is a non-negative 
integer, and it requires that before entering into this 
function, this history trace must contain the event  
\anyevent{Cup} in the tail. \codem{makeCoffee} sets a 
5 time-units deadline while calling 
\codem{addNSugar} (defined in \figref{fig:Value_dependent_intro}),
 with argument \codem{n}; then emits event \anyevent{Coffee} with
 a 3 time-units deadline. 
...
Directly from the specifications, we are aware of:  
(i) branching properties: different arithmetic conditions on the input parameters lead to different temporal effects;
and (ii) required history traces: by defining the prior effects 
in precondition. 
The examples 
already show that our  effects provide more detail information than 
...
Nevertheless, the gain in expressive power comes at the efforts 
of a more dedicated verification process, namely handled by our forward verifier and TRS.
...
\begin{figure}[!ht]
    %  \vspace{-4mm}
     \begin{minipage}[c]{\columnwidth}
         ...
         {\small
         \begin{enumerate}
\item \lstinline|void addNSugar (int n) {| \textcolor{mGray}{\footnotesize \emph{(-- initialize the current effects state --)}} \\
     \textcolor{darklavender}{\{\code{\effect_C {=}\effect_{pre}^{addNSugar(n)} =  n{\geq}0 \wedge  \_^\star \} }} 
    ...
    \item 
     ~\qquad~\qquad \lstinline|timeout ( addOneSugar() , 1);| \\
