...
%where \code{\effect^\prime} simply indicates that the signal \anyevent{Cook} needs to happen at the second event, and takes no more than \code{3} time units after the first event has completed; and \code{\_^\star} means we do not care the rest of the traces. 
...
\begin{flalign*}
  ...
  \code{\frac{
    (V,e_1) \xrightarrow[]{\anyevent{A}} (V^\prime,e_1^\prime) 
  }{(V, e_1 || e_2 ) \xrightarrow[]{\anyevent{A}} (V^\prime, e_1^\prime || e_2)}\ [pa_1]} 
  \quad  
  \code{\frac{
    (V,e_2) \xrightarrow[]{\anyevent{A}} (V^\prime,e_2^\prime) 
  }{(V, e_1 || e_2 ) \xrightarrow[]{\anyevent{A}} (V^\prime, e_1 || e_2^\prime)}\ [pa_2]} 
\quad
\code{\frac{
  (V,e_1) \xrightarrow[]{\anyevent{A}/\tau} (V^\prime,e_1^\prime) 
}{(V, e_1; e_2 ) \xrightarrow[]{\anyevent{A}/\tau} (V^\prime, e_1^\prime ; e_2)}\ [se]} 
\end{flalign*}
...
\centering
 ...
 \m{(Timed\ Effects)}  & \ \code{\effect}   &\code{ ::=}&
   
   \code{ \bott
   \  | \ \empt 
    \ | \ ev 
    %   \ | \ \anyevent{a}
   \  | \ \effect_1 {\seq} \effect_2
    \ | \ \effect_1 {\vee} \effect_2
    \ | \ \effect_1 {||} \effect_2
      %    \  | \ \effect_1 \times \effect_2
      \ | \ \effect \# t 
   \  | \  \effect^\star
}
...
\figref{fig:Syntax_of_Types_and_Effects} Effects is 
 a conditioned time  event sequence \code{\pure{\pi},  \effect} or a disjunction of two effects $\effect_1 \vee \effect_2$.
...
%while \code{\anyevent{a}.[1 ..]} refers to the infinite event sequence \code{\anyevent{a}.1 \cdot \anyevent{a}.2 \cdot \anyevent{a}.3 \cdot}..., and \code{\anyevent{a}.[1, 3..]} refers to the infinite event sequence \code{\anyevent{a}.1 \cdot \anyevent{a}.3 \cdot \anyevent{a}.5\cdot}...);
 sequences concatenation \code{\effect_1\cdot \effect_2};
disjunction  \code{\effect_1\vee \effect_2};
parallel composition of two processes is written as \code{\effect_1 || \effect_2}, where \code{\effect_1} and \code{\effect_2} 
may communicate via multi-party event synchronization or shared variables. 
We introduce a new operator \code{\#}, and the effects \code{\effect \# t } represents that a trace takes real-time \code{t} to complete, where \code{t} is a \emph{term}. 
...
\code{\effect} is the set of event sequences (or event multi-trees, per se), indicating the sequencing constraints on temporal behaviour.
...
%%         Empty          %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s,  \varphi \models \pure{\pi}  , \empt}  
&\m{iff}\quad & \code{d{=}0}\ and  \ \code{\llbracket \pure{\pi} \rrbracket_s {=}  
\pure{True} } \ and\  \code{ \varphi {=} [] }\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%         Event          %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s, \varphi \models \pure{\pi}  , ev }  
&\m{iff}\quad & \code{d{=}0}\ and  \ \code{\llbracket \pure{\pi} \rrbracket_s {=}  
\pure{True} }\ and \ \varphi {=} [ev]  \\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%     Concatenate        %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s, \varphi \models \pure{\pi}, (\effect_1\cdot\effect_2)}  
&\m{iff}\quad & \exists \code{\varphi_1,\varphi_2}. \  
\code{ \varphi_1 {\lappend} \varphi_2 {=} \varphi}\ and\ 
\exists \code{d_1,d_2}. \  
\code{ d_1 {+} d_2 {=} d} \\
&&& \qquad  s.t. \ \code{d_1, s, \varphi_1 {\models} \pure{\pi}  , \effect_1 }
\ and \ \code{d_2, s, \varphi_2 {\models} \pure{\pi}  , \effect_2 }\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%    disjunction         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s, \varphi \models \pure{\pi}  , (\effect_1{\vee}\effect_2)}  
&\m{iff}\quad & \code{d, s, \varphi \models \pure{\pi}  , \effect_1 } 
\ or\ \code{d, s, \varphi \models \pure{\pi}  , \effect_2 } \\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%    parallel         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s, \varphi \models \pure{\pi} , (ev_1{\cdot}\effect_1)||(ev_2{\cdot}\effect_2)}  
&\m{iff}\quad & \code{d, s, \varphi \models \pure{\pi}  , ev_1{\cdot} (\effect_1||(ev_2{\cdot}\effect_2)) } 
\ or\ \code{d, s, \varphi \models \pure{\pi}  , ev_2{\cdot}((ev_1{\cdot}\effect_1)||\effect_2)  } \\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%    parallel  2        %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s, \varphi \models \pure{\pi} , (ev\#t_1) || (\epsilon \# t_2)}  
&\m{iff}\quad & \code{d, s, \varphi \models \pure{(\pi{\wedge}t_1 {\geq}t_2)}, (ev\#t_1) } 
\ or\ \code{d, s, \varphi \models \pure{(\pi{\wedge}t_1 {<}t_2)}  , (ev\#t_1){\cdot}(\epsilon \# (t_2\text{-}t_1))  } \\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%    t times         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s, \varphi \models \pure{\pi}  ,\effect\# t}  &\m{iff}\quad &
 \code{\llbracket \pure{\pi}  , t {=} 0 \rrbracket_{s} {=}  
 \pure{True} }, \ \code{d, s, \varphi \models \pure{\pi}  , \empt}\ or  \\
& & &  \code{\llbracket \pure{\pi}, t {>} 0 \rrbracket_{s} {=}  
\pure{True} }, 
\  \exists \code{\effect_1,\effect_2}. \  \code{\effect_1 {\cdot} \effect_2 {=} \effect}\\
&&& \qquad  s.t. \ \code{d, s, \varphi {\models} \pure{(\pi 
{\wedge} t_1{\geq}0{\wedge}t_2{\geq}0 {\wedge} t_1{+} t_2{=}t)}  , 
(\effect_1\#t_1) {\cdot} (\effect_2\#t_2) } \\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%    star         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s, \varphi \models \pure{\pi}, \effect^\star}  &\m{iff}\quad & 
\code{d, s, \varphi \models \pure{\pi}  , \empt}\ or \ 
\code{d, s, \varphi \models \pure{\pi}  , \effect {\cdot} \effect^\star}\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%    FALSE         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%                            CONCATE                            %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, \varphi \models  \pi : (\effect_1{\cdot}\effect_2)}  
&\m{iff}\quad 
& {\tt{SAT}(\pi)}  \ and \ \exists \code{\varphi_1, \varphi_2, d_1, d_2}\ and\ \code{
\varphi {=} \varphi_1 \lappend \varphi_2, d {=} d_1 {+} d_2 \ and 
}\\
& & &
\code{d_1, \varphi_1 \models  \pi : \effect_1}  \ and \ 
\code{d_2, \varphi_2 \models  \pi : \effect_2}  
 \\[0.2em]
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%                       DISJUNCTION                         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, \varphi \models  \pi : (\effect_1 {\vee} \effect_2)}  
&\m{iff}\quad 
& {\tt{SAT}(\pi)}  \ and \ \code{d, \varphi \models  \pi :  \effect_1 } \ or \ 
 \code{d, \varphi \models  \pi :  \effect_2}  
\\[0.2em]
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%                       timed                         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, \varphi \models  \pi : \effect \# t }  
&\m{iff}\quad 
& d, \varphi \models  (\pi \wedge t{=}d) : \effect
\\[0.2em]
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%                            KLEENE                            %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
the effects \code{\effect} take exactly \code{d} time units to complete; and 
...
With pre/post conditions declared for each method in \code{\mathcal{P}}, 
we can apply modular verification to a method's body using Hoare-style 
triples \code{\ \vdash \s{ \effect, \Pi } \ e\ 
\s{\effect^\prime, \Pi^\prime}}
where 
\code{\effect} is the current effects, 
\code{\Pi} is the current guard, 
%\code{\env} is the current valuation of the variables;
and \s{\effect^\prime, \Pi^\prime}  
...
\begin{gather*}
  \code{\frac{ 
 \vdash \{ \effect, \Pi\}\ e_1 \ \{  \effect^\prime, \Pi^\prime\} 
\qquad 
\vdash \{ \effect^\prime, \Pi^\prime\}\ e_2 \ \{  \effect^{\prime\prime}, \Pi^{\prime\prime} \} 
}{
  \vdash \{  \effect, \Pi \}\  
  e_1;e_2 \ \{ \effect^{\prime\prime}, \Pi^{\prime\prime}
  \}} \ 
   {[FV\text{-}Seq]}}  
\end{gather*}
...
\begin{gather*}
 ...
 \vdash \{ \effect, \Pi \}\ e_1 \ \{  \effect^\prime, \Pi^\prime\} 
\qquad 
\vdash \{  \effect, \Pi \}\ e_2 \ \{  \effect^{\prime\prime}, \Pi^{\prime\prime} \} 
    }{
\vdash \{   \effect, \Pi \}\  e_1 || e_2 \ \{ \effect^\prime || \effect^{\prime\prime}, 
\Pi^{\prime} {\wedge}\Pi^{\prime\prime}
\}} \   
{[FV\text{-}Par]}} 
\end{gather*}
...
\begin{gather*}
  \code{\frac{ 
    \begin{matrix}
\vdash \{ \effect, \Pi {\wedge}  v   \} \ e_1 \ \{  \effect^{\prime} , \Pi^\prime \} 
\qquad \vdash \{ \effect, \Pi {\wedge} \neg v  \} \ e_2 \ 
\{  \effect^{\prime\prime}, \Pi^{\prime\prime} \}  
\\ \{  \effect_f, \Pi_f \} {=} \s{ \effect^{\prime} , \Pi^\prime} \cup \s{\effect^{\prime\prime}, \Pi^{\prime\prime}}
\end{matrix}}
  {\vdash \s{ \effect, \Pi}\  \textbf{if}\ v\ \textbf{then}\ e_1\ 
  \textbf{else}\ e_2 \ \{  \effect_f, \Pi_f \} } \  
  {[FV\text{-}If\text{-}Else]}} 
  \end{gather*}
...
\begin{gather*}
  \code{\frac{
    \effect^\prime {=} \effect\cdot \anyevent{A}(v, \Pi, \alpha^*) 
    }
  {\vdash \s{ \effect, \Pi} \  
  \textbf{event} [\anyevent{A}(v, \alpha^*)] 
   \ \{ \effect^\prime, True \}} \   {[FV\text{-}Event]}} 
   \qquad
  \code{\frac{
    \effect^\prime {=} \effect\cdot \epsilon\#t \quad (t\ is\ fresh) 
    }
  {\vdash \s{ \effect, \Pi} \  
  \textbf{delay} [d] 
   \ \{ \effect^\prime, True \}} \   {[FV\text{-}Delay]}} 
\end{gather*}
...
\begin{gather*}
  \code{\frac{
    \begin{matrix}
      \vdash \{ \epsilon, \Pi \}\ e_1 \ \{  \effect^\prime, \Pi^\prime\} 
      \qquad 
      \vdash \{  \epsilon, \Pi \}\ e_2 \ \{  \effect^{\prime\prime}, \Pi^{\prime\prime} \} 
      \\
      \s{\effect_f, \Pi_f} {=} \s{\effect {\cdot}(\effect^\prime\#t), \Pi^\prime {\wedge}t{\leq}d}
      \cup 
  \s{\effect {\cdot}(\effect^{\prime\prime}\#t), \Pi^{\prime\prime} {\wedge}t{>}d}   
  \quad (t\ is\ fresh)     
\end{matrix}
  }
  {\vdash \s{ \effect, \Pi} \  
  e_1\ \textbf{timeout} [d] \  e_2
   \ \s{\effect_f, \Pi_f} } \   {[FV\text{-}Timeout]}} 
\end{gather*}

\begin{gather*}
  \code{\frac{
    \begin{matrix}
      \vdash \{ \epsilon, \Pi \}\ e \ \{  \effect^\prime, \Pi^\prime\} 
      \qquad 
      \s{\effect_f, \Pi_f} {=} \s{\effect {\cdot}(\effect^\prime\#t), \Pi^\prime {\wedge}t{\leq}d}
  \quad (t\ is\ fresh)     
\end{matrix}
  ...
  {\vdash \s{ \effect, \Pi} \  
  \textbf{deadline}  [d] \ e 
   \ \s{\effect_f, \Pi_f} } \   {[FV\text{-}Deadline]}} 
\end{gather*}
...
\code{\frac{
\begin{matrix}
\text{\code{  mn  \ {x^*}\ \{\textbf{requires}\ \effect_{pre}\ \textbf{ensures}\  \effect_{post}\} \ \{ e \} \in \mathcal{P}\ \ }}
\\
\text{\code{\vdash  \effect \CONTAIN [v^*/x^*] \effect_{pre}   \qquad   \effect^\prime =  \effect \cdot  [y^*/x^*]\effect_{post}}}
\end{matrix}}
{\vdash \s{ \effect, \Pi} \ mn({v^*})  \ 
\{  \effect^\prime,  \Pi\}} \ {[FV\text{-}Call]}}
\end{gather*}

\begin{gather*}
\code{\frac{  \vdash \{  \empt, True \} 
 \ e \  \{ \effect, \Pi \} \qquad\qquad  \vdash\effect \CONTAIN  
  \effect_{post}}{  \vdash  \ mn  \ {x^*}\ \{\textbf{requires}\ \effect_{pre}\ \textbf{ensures}\  \effect_{post}\} \ \{ e \}} \ {[FV\text{-}Meth]}}  
\end{gather*}
...
Intuitively, the Nullable function \code{\delta(\effect)} returns a boolean value indicating whether \code{\effect} contains the empty trace; the First function \code{fst_{\pure{\pi}}( \effect)} computes a set of possible initial time instances of \code{\pure{\pi} : \effect}; and the Derivative function \code{D_{I\# t}^{\pure{\pi}}(\effect)} computes a next-state effects after eliminating one event \code{I} w.r.t. the execution time \code{t} from the current effects \code{\pure{\pi} : \effect}. 
...
Given any time sequence \code{\effect}, we recursively define \code{\delta(\effect)} as:\\
{
\code{\delta(\effect) :bool {=}
\begin{cases}
      \code{true} & \code{if\ \empt \in \effect}\\
...
\qquad\quad\
    \code{\delta({\effect^\star} ) {=} true}   
\\ 
\code{\delta(\effect_1 \seq \effect_2) {=} \delta(\effect_1) \wedge \delta(\effect_2)}
\qquad\qquad
  \code{\delta(\effect_1 \choice \effect_2) {=} \delta(\effect_1) \vee \delta(\effect_2)}   
  \\
\code{\delta(\effect_1 || \effect_2) {=} \delta(\effect_1) \wedge \delta(\effect_2)}
\qquad\qquad\quad
 \code{\delta(\effect \# t) {=} \delta(\effect)}
\end{gather*}}
\end{definition}
...
Let \code{fst^\prime(\effect){:=}\{ I\ |\  (I \cdot \effect^\prime) \in \llbracket  \effect \rrbracket  \}} be the set of initial instances derivable from  sequence \code{\effect}. (\code{\llbracket  \effect \rrbracket } represents all the traces contained in \code{\effect}.)
{ 
 \begin{gather*} 
...
\code{fst^\prime( I) {=} \{ I \} }
 \qquad\ 
\code{fst^\prime(  \effect_1 {\vee} \effect_2) {=} fst^\prime(  \effect_1) \cup fst^\prime(  \effect_2)}
\\
{
\code{fst^\prime( {\effect^\star} ) {=} fst^\prime({ \effect})} 
\qquad\quad\
\code{fst^\prime(\effect_1 {\seq} \effect_2) {=} } 
\begin{cases}
      \code{\code{fst^\prime(  \effect_1)} \cup \code{fst^\prime(  \effect_2)} } \quad & \code{ if\ \delta(\effect_1) {=} true}\\
      \code{\code{fst^\prime(  \effect_1)}} & \code{  if\ \delta(\effect_1) {=} false}
    \end{cases} 
    }
\end{gather*}
}
...
Let \code{fst_{\pure{\pi}}(\effect){:=}\{ (I,  \pi, t) \ |\  \pi {\wedge} t{\geq}0 : (I\# t) \cdot \effect^\prime \in \llbracket  \pi {:} \effect \rrbracket \}} be the set of initial time instances derivable from effects \code{\pi {:} \effect}. (\code{\llbracket  \pi {:} \effect \rrbracket } represents all the time  traces contained in \code{\pi {:} \effect}) 
{ 
 \begin{gather*} 
...
\qquad\quad
   \code{fst_{\pure{\pi}}(  \effect \# t) {=}  fst_{pi}(\effect)}
 \\
\code{fst_{\pure{\pi}}( {\effect^\star} ) {=} fst_{\pure{\pi}}({ \effect})}
\qquad\quad
\qquad\quad
\code{fst_{\pure{\pi}}(  \effect_1 {\vee} \effect_2) {=} fst_{\pure{\pi}}(  \effect_1) \cup fst_{\pure{\pi}}(  \effect_2)  }  \\
\code{fst_{\pure{\pi}}( \textbf{S}? ) {=} fst_{\pure{\pi}}( \{\textbf{S}\} ) \cup fst_{\pure{\pi}}( \{ \overline{\textbf{S}} \} ) }
    \qquad\qquad\quad
   \code{fst_{\pure{\pi}}(  \effect_1 {||} \effect_2) {=} unify \ 
    (fst_{\pure{\pi}}(  \effect_1)) \  (fst_{\pure{\pi}}(  \effect_2))  }  
   \\
{
\code{fst_{\pure{\pi}}(\effect_1 {\seq} \effect_2) {=} } 
\begin{cases}
      \code{\code{fst_{\pure{\pi}}(  \effect_1)} \cup \code{fst_{\pure{\pi}}(  \effect_2)} } & \code{ if\ \delta(\effect_1) {=} true}\\
      \code{\code{fst_{\pure{\pi}}(  \effect_1)}} & \code{  if\ \delta(\effect_1) {=} false}
    \end{cases} 
    }  
\end{gather*}
}
...
\code{False : \bott} 
\qquad \ 
\code{D^{\pi}_{(I, \pi^\prime, t^\prime)}(\effect_1 ||\effect_2)} {=} \code{D^{\pi}_{(I, \pi^\prime, t^\prime)}(\effect_1)} || \code{D^{\pi}_{(I, \pi^\prime, t^\prime)}(\effect_2)} 
    \\
        \code{D^{\pi}_{(I, \pi^\prime, t^\prime)}(  {\effect^\star}) } {=} \code{ D^{\pi}_{(I, \pi^\prime, t^\prime)}(  {\effect}) \cdot ({\effect^\star})}  
        \qquad\qquad
\\
{
 \code{D^{\pi}_{(I, \pi^\prime, t^\prime)}(   \effect_1 \seq \effect_2) {=} }
\begin{cases}
      \code{D^{\pi}_{(I, \pi^\prime, t^\prime)}(   \effect_1) \cdot  \effect_2 \vee  D^{\pi}_{(I, \pi^\prime, t^\prime)}( \effect_2)} \quad &\code{if\ \delta(\effect_1) {=} true}   \\
      \code{D^{\pi}_{(I, \pi^\prime, t^\prime)}  ( \effect_1) \cdot  \effect_2} & \code{if\ \delta(\effect_1) {=} false} 
    \end{cases} 
    }
 \\
\code{D^{\pi}_{(I, \pi^\prime, t^\prime)}(  \effect_1 \choice \effect_2) } {=} \code{ D^{\pi}_{(I, \pi^\prime, t^\prime)}(  \effect_1) \choice D^{\pi}_{(I, \pi^\prime, t^\prime)}(  \effect_2)} 
%\\
%\code{D^{\pi}_{(I, \pi^\prime, t^\prime)}(  \effect_1 \ ||\  \effect_2) } {=} \code{ D^{\pi}_{(I, \pi^\prime, t^\prime)}(  \effect_1) \ ||\ D^{\pi}_{(I, \pi^\prime, t^\prime)}(  \effect_2)} 
\\
\code{D^{\pi}_{(I, \pi^\prime, t^\prime)}(  \effect \# t) } {=}
\code{
\pi \wedge(t_1{+} t_2 {=}t) \wedge (t_1{=}t^\prime) : (D^{\pi}_{(I, \pi^\prime, t^\prime)}(\effect))\# t_2} \ \ \ 
\code{(fresh \ t_1\ t_2 )}
\\
...
\hline
(A17)  &
 \code{(I\# t) \cdot \effect_1\ ||\ J \cdot\effect_2 \rightarrow ((I  {\cup} J) \# t) \cdot (\effect_1 || \effect_2)}\\
\hline
(A18)  &
\code{\pi : (\effect \# t_1 ) \# t_2 \rightarrow \pi  \wedge (t_1 {=} t_2) : \effect \# t_1 }\\
\hline
(A19)  &
...
\hline
(A21)  &
 \code{{\pure{False}} : \effect \rightarrow {\pure{False}} : \bott}\\
\hline
(A22)  &
...
%\begin{table}[ht]
%        \vspace{-2mm}
%\caption{\label{tab:Experimental} The experiments are based on 16 real world C programs, we record the lines of code (LOC), the number of testing temporal properties ($\#$Prop.), and the (dis-) proving times (in time units) using PAT and our T.r.s respectively. }
%\centering
%\footnotesize
%\renewcommand{\arraystretch}{1.1}
