...
\code{(n{>}0  \wedge  t {\geq} n) : ({\anyevent{Sugar} \mysharp  t)}}
...
%%    disjunction         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s, \varphi \models \pure{\pi}, \Pi_1.(\es_1{\vee}\es_2)}  
&\m{iff}\quad & 
\code{\llbracket \Pi_1  \rrbracket_{s} {=}  
 \pure{True} },\code{d, s, \varphi \models \pure{\pi}  , \es_1 } 
\ or\ 
\code{\llbracket \Pi_1  \rrbracket_{s} {=}  
 \pure{False} },
\code{d, s, \varphi \models \pure{\pi}  , \es_2 } \\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%    parallel         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%    parallel  2        %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s, \varphi \models \pure{\pi} , (\ev\mysharp t_1) || (\epsilon \mysharp  t_2)}  
&\m{iff}\quad & \code{d, s, \varphi \models \pure{(\pi{\wedge}t_1 {\geq}t_2)}, (\ev\mysharp t_1) } 
\ or\ \code{d, s, \varphi \models \pure{(\pi{\wedge}t_1 {<}t_2)}  , (\ev\mysharp t_1){\cdot}(\epsilon \mysharp  (t_2\text{-}t_1))  } \\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%    block         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s, \varphi \models \pure{\pi}, \Pi_1 ? es}  &\m{iff}\quad & 
\code{\llbracket \Pi_1  \rrbracket_{s} {=}  
 \pure{True} }, \ \code{d, s, \varphi \models \pure{\pi}  , es}\ or  \ 
  \code{\llbracket \Pi_1 \rrbracket_{s} {=}  
\pure{False} }, 
\  \code{d, s, \varphi \models \pure{\pi}, \Pi_1 ? es}\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%    t times         %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
&\code{d, s, \varphi \models \pure{\pi}  ,es\mysharp  t}  &\m{iff}\quad &
...
\s{\pi_i^\prime, \es_i^\prime} \ \ where\ \ 
 \vdash  \s{\pi_i, \es_i} \ p \ \s{\pi_i^\prime, \es_i^\prime}.} 
...
\vdash \{   \Pi, \Theta \}\  e_1 || e_2 \ \{ \Theta_1 || \Theta_2, 
\Pi^{\prime} {\wedge}\Pi_2
\}}  
{[FV\text{-}Par]}} 
\end{gather*}}}
...
{{\small\begin{gather*}
  \code{\frac{ 
    \begin{matrix}
\vdash \{ \Pi, \Theta {\wedge}  v   \} \ e_1 \ \{  \Theta^{\prime} , \Pi_1 \} 
\qquad \vdash \{ \Pi, \Theta {\wedge} \neg v  \} \ e_2 \ 
\{  \Pi_2, \Theta_2 \}  
\qquad 
 \{  \Theta_f, \Pi_f \} {=} \s{ \Theta^{\prime} , \Pi_1} \cup \s{\Pi_2, \Theta_2}
\end{matrix}}
  {\vdash \s{ \Pi, \Theta}\  \textbf{if}\ v\ \textbf{then}\ e_1\ 
  \textbf{else}\ e_2 \ \{  \Theta_f, \Pi_f \} } \  
  {[FV\text{-}If\text{-}Else]}} 
  \end{gather*}}}
  ...
  {{\small\begin{gather*}
  \code{\frac{
    \Theta_1 {=} \Theta\cdot \anyevent{A}(v, \Pi, \alpha^*) 
    }
  {\vdash \s{ \Pi, \Theta} \  
  \textbf{event} [\anyevent{A}(v, \alpha^*)] 
   \ \{ \Theta_1, True \}} \   {[FV\text{-}Event]}} 
   \qquad
  \code{\frac{
    \Theta_1 {=} \Theta\cdot \epsilon\mysharp t \quad (t\ is\ fresh) 
    }
  {\vdash \s{ \Pi, \Theta} \  
  \textbf{delay} [d] 
   \ \{ \Theta_1, True \}} \   {[FV\text{-}Delay]}} 
\end{gather*}}}
...
{{\small\begin{gather*}
  \code{\frac{
    \begin{matrix}
      ...
      \\
      \s{\Theta_f, \Pi_f} {=} \s{\Theta {\cdot}(\Theta_1\mysharp t), \Pi_1 {\wedge}t{\leq}d}
      \cup 
  \s{\Theta {\cdot}(\Theta_2\mysharp t), \Pi_2 {\wedge}t{>}d}   
  \quad (t\ is\ fresh)     
\end{matrix}
  }
  {\vdash \s{ \Pi, \Theta} \  
...
\end{matrix}
  ...
  {\vdash \s{ \Pi, \Theta} \  
  \textbf{deadline}  [d] \ e 
   \ \s{\Theta_f, \Pi_f} } \   {[FV\text{-}Deadline]}} 
\end{gather*}}}
...
\code{\frac{
\begin{matrix}
{\code{  mn  \ {x^*}\ \{\textbf{req}\ \Theta_{pre}\ \textbf{ens}\  \Theta_{post}\} \ \{ e \} \in \mathcal{P}\ \ }}
\\
\text{\code{\vdash  \Theta \CONTAIN [v^*/x^*] \Theta_{pre}   \qquad   \Theta_1 =  \Theta \cdot  [y^*/x^*]\Theta_{post}}}
\end{matrix}}
{\vdash \s{ \Pi, \Theta} \ mn({v^*})  \ 
\{  \Theta_1,  \Pi\}} \ {[FV\text{-}Call]}}
\ \quad 
\code{\frac{  \vdash \{  \empt, True \} 
 \ e \  \{ \Pi, \Theta \} \quad  \vdash\Theta \CONTAIN  
  \Theta_{post}}{  \vdash  \ mn  \ {x^*}\ \{\textbf{req}\ \Theta_{pre}\ 
  \textbf{ens}\  \Theta_{post}\} \ \{ e \}} \  {[FV\text{-}Meth]}}  
\end{gather*}}}
...
Intuitively, the Nullable function \code{\delta(\effect)} returns a boolean value indicating whether \code{\effect} contains the empty trace; the First function \code{fst_{\pure{\pi}}( \effect)} computes a set of possible initial time instances of \code{\pure{\pi} : \effect}; and the Derivative function \code{D_{I\mysharp  t}^{\pure{\pi}}(\effect)} computes a next-state effects after eliminating one event \code{I} w.r.t. the execution time \code{t} from the current effects \code{\pure{\pi} : \effect}. 
...
\qquad\qquad\quad
 \code{\delta(\effect \mysharp  t) {=} \delta(\effect)}
\end{gather*}}
\end{definition}
...
Let \code{fst^\prime(\effect){:=}\{ I\ |\  (I \cdot \effect_1) \in \llbracket  \effect \rrbracket  \}} be the set of initial instances derivable from  sequence \code{\effect}. (\code{\llbracket  \effect \rrbracket } represents all the traces contained in \code{\effect}.)
{ 
 \begin{gather*} 
...
the real-time bounds constructor \code{\mysharp }. Next, we define the novel  \emph{First} function used in this work, denoted using \code{fst}. 
%but helps to build up our \emph{First} function \code{fst}. 

\begin{definition}[First]\label{First}
Let \code{fst_{\pure{\pi}}(\effect){:=}\{ (I,  \pi, t) \ |\  \pi {\wedge} t{\geq}0 : (I\mysharp  t) \cdot \effect_1 \in \llbracket  \pi {:} \effect \rrbracket \}} be the set of initial time instances derivable from effects \code{\pi {:} \effect}. (\code{\llbracket  \pi {:} \effect \rrbracket } represents all the time  traces contained in \code{\pi {:} \effect}) 
{ 
 \begin{gather*} 
...
\qquad\quad
   \code{fst_{\pure{\pi}}(  \effect \mysharp  t) {=}  fst_{pi}(\effect)}
 \\
...
%\\
%\code{D^{\pi}_{(I, \Pi_1, t^\prime)}(  \effect_1 \ ||\  \effect_2) } {=} \code{ D^{\pi}_{(I, \Pi_1, t^\prime)}(  \effect_1) \ ||\ D^{\pi}_{(I, \Pi_1, t^\prime)}(  \effect_2)} 
\\
