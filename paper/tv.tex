...
{  \vdash \{  \effect,\Pi, \env \}\  v \ 
\{ \effect, \Pi, \env \}} \  
 {[FV\text{-}Value]}} 
 \qquad
 \code{\frac{
  }
{  \vdash \{  \effect, \Pi, \env \}\  [b]\ e \ 
\{ \effect, \Pi{\wedge} b, \env  \}} \  
 {[FV\text{-}Guard]}} 
\end{gather*}
...
\begin{gather*}
  \code{\frac{ 
 \vdash \{ \effect, \Pi, \env\}\ e_1 \ \{  \effect^\prime, \Pi^\prime, \env^\prime\} 
\qquad 
\vdash \{ \effect^\prime, \Pi^\prime, \env^\prime\}\ e_2 \ \{  \effect^{\prime\prime}, \Pi^{\prime\prime}, \env^{\prime\prime} \} 
}{
  \vdash \{  \effect, \Pi, \env \}\  
...
\begin{gather*}
 ...
 \vdash \{ \effect, \Pi, \env \}\ e_1 \ \{  \effect^\prime, \Pi^\prime, \env^\prime\} 
\qquad 
\vdash \{  \effect, \Pi, \env \}\ e_2 \ \{  \effect^{\prime\prime}, \Pi^{\prime\prime}, \env^{\prime\prime} \} 
    }{
\vdash \{   \effect, \Pi, \env \}\  e_1 || e_2 \ \{ \effect^\prime || \effect^{\prime\prime}, 
...
\begin{gather*}
  \code{\frac{\effect^\prime {=} \effect\cdot \anyevent{a}}{\vdash \{  \effect \}\  \textbf{event} [\anyevent{a}] \ \{ \effect^\prime \}} \   {[FV\text{-}Event]}} 
\end{gather*}
...
\begin{gather*}
\code{\frac{ \vdash \{  v \wedge \effect  \} \ e_1 \ \{  \effect^{\prime}\} \qquad \vdash \{ \neg v \wedge \effect \} \ e_2 \ \{  \effect^{\prime\prime}\}  }
{\vdash \{  \effect\}\  \textbf{if}\ v\ \textbf{then}\ e_1\ \textbf{else}\ e_2 \ \{  \effect^{\prime} \vee\effect^{\prime\prime} \} } \  {[FV\text{-}If\text{-}Else]}} 
\end{gather*}

\begin{gather*}
\code{\frac{
\begin{matrix}
\text{\code{  \tau \ mn  \ {( \tau\ x)^*}\ \{\textbf{requires}\ \effect_{pre}\ \textbf{ensures}\  \effect_{post}\} \ \{ e \} \in \mathcal{P}\ \ }}
\\
\text{\code{\vdash rev( \effect) \CONTAIN rev([y^*/x^*] \effect_{pre})   \qquad   \effect^\prime =  \effect \cdot  [y^*/x^*]\effect_{post}}}
\end{matrix}}
{\vdash \{  \effect \} \  mn({y^*})  \ \{  \effect^\prime\}} \ {[FV\text{-}Call]}}
\end{gather*}

\begin{gather*}
\code{\frac{  \vdash \{  \empt \}  \ e \  \{ \effect \} \qquad\qquad  \vdash\effect \CONTAIN   \effect_{post}}{  \vdash \tau \ mn  \ {( \tau\ x)^*}\ \{\textbf{requires}\ \effect_{pre}\ \textbf{ensures}\  \effect_{post}\} \ \{ e \}} \ {[FV\text{-}Meth]}}  
\end{gather*}
