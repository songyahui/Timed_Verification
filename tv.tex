...
compositional real-time systems, which are modeled 
using mutable variables and timed processes.  
Instead of explicitly manipulating clock variables, 
a number of compositional timed behavioral patterns, 
...
More specifically, we formally define 
a core language \timedL, generalizing the timed systems.
Secondly, we introduce \timedEffects, a new effects logic, 
that extends 
\emph{Kleene Algebra} with value-dependent real-time constraints. 
Thirdly,  we establish an  axiomatic semantics for  
\timedL\  to infer temporal behaviors  of  given 
programs, expressed in \timedEffects. 
...
correctness; report case studies and the experimental results. 
%we prototype the  verification system in OCaml; then prove soundness of both the effects inference process and the term rewriting process. % in Coq. 
...
\begin{enumerate}
\item \textbf{The Timed Effects:} We define the syntax ({\secref{subsec:Specification_language}}) and semantics ({\secref{subsec:Specification_Semantics}}) of timed synchronous effects, to be the specification language, which captures the target programs' behaviours and non-trivial temporal properties.
